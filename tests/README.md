...
**`modificar-cancelar-turno.spec.js`** — el segundo bloque de Donante, agregado 2026-09-01, ajustado 2026-09-16 y 2026-09-22:
...
3. **Agregado 2026-09-22:** el banner de "próximo turno" de `mis_turnos.html` está visible mientras el turno sigue activo, y desaparece al cancelarlo si no queda ningún otro turno próximo — cubre un bug real encontrado de paso durante la limpieza de estilos inline (`renderMisTurnos()` no tenía un `else` para volver a ocultar el banner, así que cancelar el único turno próximo lo dejaba visible y vacío).
...
7. **Agregado 2026-09-23:** contestar una sola pregunta del cuestionario (sin que autoexcluya) muestra un aviso listando las que todavía faltan por número ("Todavía te faltan responder 33 preguntas" + la lista) — antes el botón solo quedaba deshabilitado sin explicación. El aviso desaparece al completar las 34. **Ajustado el mismo día, reportado por la usuaria:** este aviso y el de "respuesta inhabilitante" (caso de prueba nuevo más abajo) tienen que verse juntos si ambas condiciones se dan a la vez — antes el de "faltan preguntas" se ocultaba si había una inhabilitante, y recién aparecía al corregirla, obligando a revisar el formulario 2 veces.
8. **Agregado 2026-09-23:** en "Mis turnos", con ambos formularios ya completados, el botón pasa a decir "Ver formularios" en vez de "Completar formularios".
...
4. Clickear una notificación la marca como leída de verdad (persiste tras recargar la página); "Marcar todas como leídas" hace lo mismo para todas. **Ajustado 2026-09-23:** este botón pasó de texto a un ícono en la topbar (mismo estilo `.topbar-btn` que el resto de las pantallas — ver `docs/04`) — el test lo ubica por su `data-tooltip`, no por texto visible.
...
**`mostrar-contrasena.spec.js`** — botón de mostrar/ocultar contraseña, agregado 2026-09-22 (hito acordado con el profesor). No es de un rol en particular — `HemoRed.ui.mejorarPasswords()` envuelve automáticamente todos los `<input type="password">` del sitio, mismo criterio que el select personalizado:

1. En `login.html`, el botón alterna el campo entre oculto (`type="password"`) y visible (`type="text"`) sin perder lo ya tipeado.
2. Aparece en los 4 campos de `registro.html` (contraseña + confirmar, para donante y para institución) y en los 2 de `recuperar.html` (a esta pantalla, que no cargaba ningún script de HemoRed, hubo que agregarle `db.js` y `ui.js`).
3. Aparece en los 3 modales de "Seguridad de la cuenta" (`perfil.html`): cambiar contraseña (3 campos), cambiar email (1) y eliminar cuenta (1).
4. El campo "CVV" del mockup de `pago.html` (mismo `type="password"` para taparse, pero no es una contraseña) queda afuera a propósito, marcado con `data-sin-toggle="1"`.

**`sidebar-mobile.spec.js`** — menú lateral (sidebar), agregado 2026-09-23 para Donante tras una auditoría de responsive, **reescrito 2026-09-24 para cubrir los 4 roles con sesión** (Donante, Hospital, Profesional, Admin — un `describe` por rol, mismos 4 casos) cuando se unificó el diseño del sidebar entre los 4 (ver `docs/04`, "Sidebar unificado en los 4 roles"). El sidebar fijo (240px) recién colapsaba a menú hamburguesa por debajo de los 768px — en tablet (820px) se mostraba completo y apretaba el contenido, sin forma de ocultarlo. El breakpoint pasó a 1024px, se agregó un botón X para cerrar el menú (antes solo se podía tocar afuera, sin ningún control visible), y scroll interno si el contenido no entra (ninguno de los 4 roles lo tenía):

1. En desktop (1280px) el sidebar está siempre visible, sin botón de menú ni de cerrar.
2. En tablet (820px) arranca colapsado; el botón de menú lo abre, la X lo cierra.
3. En mobile (375px) tocar el overlay oscurecido también cierra el menú (comportamiento que ya existía, se deja cubierto).
4. Con una ventana baja (420px de alto) el sidebar nunca excede el viewport; si el menú de ese rol es lo bastante largo como para no entrar, se puede scrollear hasta ver el último ítem.

**`responsive-320.spec.js`** — sin scroll horizontal en las pantallas de Donante a 320px, agregado 2026-09-23, misma auditoría que `sidebar-mobile.spec.js`. La primera pasada de la auditoría usó capturas `fullPage` de Playwright a 375px, que **esconden el desborde horizontal en vez de mostrarlo** (una captura `fullPage` se ajusta al ancho real del contenido aunque sea más ancho que el viewport, así que se ve "prolijo" en la imagen aunque en un navegador real hubiera scroll horizontal) — la usuaria insistió en que seguía viendo vistas rotas en mobile, lo cual llevó a cambiar de método: comparar `document.documentElement.scrollWidth` contra el viewport directamente, y probar también a 320px (el celular más chico todavía común), no solo 375px. La causa de fondo no era cosmética: a `.main` (el contenedor de layout de toda página de Donante) le faltaba `min-width:0` — sin eso, cualquier texto sin ajustar de línea en cualquier rincón de la página (ej. la fecha del banner de "próximo turno") agrandaba el `<body>` entero en vez de quedarse contenido en su propio elemento. Ver `docs/04-estado-actual-prototipo.md`, "Auditoría de responsive — rol Donante", segunda vuelta, para el detalle completo de los 6 puntos corregidos:

1. Las 5 pantallas principales (dashboard, mis_turnos, mis_donaciones, mis_documentos, perfil, notificaciones) no tienen scroll horizontal a 320px con la cuenta demo.
2. La tarjeta de turno con el cuadro de "Formularios pre-donación" (fecha + hospital + badge + botones + 2 pastillas de estado) tampoco, el caso puntual que disparó el hallazgo.
3. Los 4 pasos del wizard completo de reserva de turno (incluido el indicador de pasos con las 4 etiquetas, el más ancho de todos) tampoco.
4. **Agregado 2026-09-23 (2), tras una captura real de la usuaria:** en `perfil.html` a 320px, el nombre/email del header no queda superpuesto con la barra de "Perfil completado", y los campos del formulario (Nombre/Apellido, Email/Teléfono, etc.) se muestran uno debajo del otro, no en 2 columnas apretadas. Este caso **no se detecta comparando `scrollWidth`** (dos elementos superpuestos no generan scroll horizontal) — compara `boundingBox()` de los elementos reales. La causa de fondo tampoco era de diseño: varias reglas de `@media` para mobile en `donante.css` estaban ubicadas ANTES en el archivo que las reglas base que debían anular — con igual especificidad gana la que aparece más abajo, así que la regla base "ganaba" de nuevo en silencio. Se movió todo el bloque de reglas de mobile al final del archivo (ver el comentario ahí, y `docs/04-estado-actual-prototipo.md`, tercera vuelta de esta misma auditoría).
5. **Agregado 2026-09-23 (3), tras 5 capturas más de la usuaria ("no miraste todas las pantallas"):** las 3 vistas de detalle de `mis_documentos.html` a las que solo se llega clickeando "Ver" desde una tarjeta (resultado de análisis, evaluación clínica, certificado) — nunca se habían abierto en ninguna vuelta anterior de esta auditoría. La grilla de 2 columnas de la evaluación clínica (signos vitales) estaba armada con un `style=""` inline, totalmente fuera del alcance de cualquier regla de CSS — se le agregó una clase (`.signos-vitales-grid`) para poder darle el mismo tratamiento que al resto.
6. **Agregado 2026-09-23 (3):** el topbar de `campana_detalle.html` (back-link + separador + título + ícono de notificaciones) no desborda sin importar qué tan largo o corto sea el título del paso actual — antes el título nunca se achicaba, así que alcanzaba con estar justo en el límite (pasaba en el paso "Confirmación", más corto, pero no en "Detalle de campaña", más largo, por una diferencia de apenas 8px).
