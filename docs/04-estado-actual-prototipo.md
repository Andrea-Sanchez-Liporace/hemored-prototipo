...
1. **Mis donaciones** (`cargarMisDonaciones()`). Solo lectura, sin escritura — el de menor riesgo de los que quedan.
2. **Mis documentos** (cablear `cargarMisDocumentos()` que ya existe y funciona + `solicitarCertificado()`).
3. **Formulario post-donación anónimo F4** (`validarTokenPostdonacion()`, `guardarRespuestaPostdonacion()`). Particular: tiene que funcionar sin sesión, por el anonimato.
4. **Notificaciones**. El más grande: `mensajes.json` no contempla notificaciones de donante hoy (solo hospital↔admin), hay que decidir el modelo de datos antes de codear.
