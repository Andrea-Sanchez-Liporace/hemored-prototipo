...
Quedan 3 flujos del rol donante sin conectar. Este es el orden sugerido para retomar, y por qué:

1. **Mis documentos** (cablear `cargarMisDocumentos()` que ya existe y funciona + `solicitarCertificado()`).
2. **Formulario post-donación anónimo F4** (`validarTokenPostdonacion()`, `guardarRespuestaPostdonacion()`). Particular: tiene que funcionar sin sesión, por el anonimato.
3. **Notificaciones**. El más grande: `mensajes.json` no contempla notificaciones de donante hoy (solo hospital↔admin), hay que decidir el modelo de datos antes de codear.
...
| `formulario_consentimiento` | Donante: `guardarFormularioConsentimiento()` (F1+F2) — 🟢. Profesional: F3 (evaluación clínica, extendería el mismo registro con `donacion_id`/firmas del profesional) — 🔴 no implementado. | Nadie todavía lo lee fuera de la propia función de guardado (ni "Mis documentos" — próximo ítem — lo consulta hoy). |
| `documentos` | Nadie. Hospital: `cargarResultadoAnalisis()`/`emitirCertificado()` — 🔴. | Donante: `cargarMisDocumentos()` **ya existe y funciona** — 🟡 lectura silenciosa, `mis_documentos.html` no la invoca todavía (es el próximo ítem de la lista). |
| `resultado_analisis` / `certificado_donacion` | Nadie. Hospital — 🔴 en ambas. | Nadie todavía — ninguna función lee estas tablas hoy; hay que agregarlas al cablear "Mis documentos". |
