...
Queda 1 flujo del rol donante sin conectar (más el pendiente de "validación de email y DNI" de arriba, bloqueado hasta que haya investigación propia — "restricciones de elegibilidad para reservar turno" también sigue pendiente de diseño pero sin ese bloqueo). "Notificaciones" ya no está en esta lista: se adelantó y se cerró junto con "Solicitudes de corrección" (ver esa sección), que la necesitaba para el aviso de rechazo.
...
**Verificado contra el código real (2026-09-08), última revisión tras cerrar "Notificaciones"** — existen funciones de escritura reales para: `usuarios` (Donante, su propio perfil/registro; Hospital, al aprobar una corrección de nombre/apellido/DNI de un certificado), `turnos` (Donante crea, Hospital confirma/rechaza), `formulario_consentimiento` (Donante, F1/F2 — sin escritura de Hospital, "Solicitudes de corrección" es exclusivo de certificados), `certificado_donacion` (Hospital, al aprobar una corrección de sus campos), `solicitudes_correccion` (Donante crea, Hospital resuelve) y `notificaciones_donante` (Hospital, solo al rechazar una solicitud de corrección — ver esa fila). Ninguna otra tabla (`campanas`, `donaciones`, `hospitales`, `documentos`, `resultado_analisis`, `mensajes`, `profesionales`, `facturas`, etc.) tiene una sola función que la escriba en ningún rol — son 100% datos semilla estáticos hasta que se conecte alguna.
...
| `notificaciones_donante` | Nadie escribe notificaciones "genéricas" todavía (turno confirmado, resultado cargado, etc. siguen siendo semilla). Hospital: `resolverSolicitudCorreccion()` (rechazo) — 🟢, único caso real hoy. | Donante: `cargarNotificacionesDonante()` (`notificaciones.html`) — 🟢. |
