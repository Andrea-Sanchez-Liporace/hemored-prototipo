...
### Pendiente de diseño: solicitudes de corrección sobre certificados y consentimientos (anotado 2026-09-08, no implementado)

**Origen:** al conectar "Mis documentos", el botón "Enviar observación" (en las vistas de certificado y consentimiento) quedó funcional del lado del donante — valida el texto y confirma con un toast — pero **no persiste nada**: no existe ninguna tabla para guardar esa observación. La usuaria pidió resolverlo bien en vez de dejarlo como texto libre, porque un campo de texto libre no es práctico para que el Hospital procese la corrección.

**Diseño acordado (UX):** en vez de un textarea, un modal con un checkbox por cada campo "reportable" del documento. Al marcar un campo, se despliegan dos valores lado a lado: el valor actual (de solo lectura, tal como está en el documento) y un campo en blanco para que el donante escriba la corrección que propone.

**Campos reportables por tipo de documento:**
- Certificado: nombre del donante, DNI, fecha de donación, volumen donado, profesional a cargo, hospital.
- Consentimiento: nombre del donante, DNI, fecha de firma, hospital.

**Nota abierta sin resolver todavía:** nombre y DNI en rigor viven en `usuarios` (el perfil), no en el documento puntual — si están mal, el lugar "correcto" para corregirlos sería el perfil, no un certificado específico. Se decidió (a confirmar cuando se implemente) dejarlos igual como reportables acá, unificado en un solo lugar donde el donante reporta cualquier error, en vez de mandarlo a editar el perfil por separado.

**Tabla nueva propuesta: `solicitudes_correccion`** — `id (PK)`, `usuario_id (FK)`, `documento_id (FK a documentos)`, `campos` (lista de `{ campo, valor_actual, valor_propuesto }`, uno por cada campo marcado), `estado (enum: pendiente | aprobada | rechazada)`, `fecha_solicitud`, `resuelto_por (FK, null hasta resolver)`, `fecha_resolucion (null hasta resolver)`, `motivo_rechazo (opcional)`.

**Es un flujo NUEVO de 2 roles, no solo una función más del donante:** el Donante reporta (escribe en `solicitudes_correccion`), pero hoy **no existe ninguna pantalla del lado del Hospital** para revisar esas solicitudes, comparar valor actual vs. propuesto, y aprobar (lo que además debería actualizar el dato real en `certificado_donacion`/`formulario_consentimiento`, no solo marcar la solicitud) o rechazar con un motivo. Ese lado falta diseñar en detalle cuando se aborde (qué pantalla de Hospital, si es una sección nueva o se suma a `hospital/documentacion.html`).

**Alcance acordado 2026-09-08:** se retoma en una sesión futura, empezando por el lado Donante (modal + guardar la solicitud como pendiente, reemplazando el toast falso actual) y después el lado Hospital (revisar/aprobar/rechazar).

### Próximos pasos — Donante (orden sugerido)

Quedan 2 flujos del rol donante sin conectar. Este es el orden sugerido para retomar, y por qué:

1. **Formulario post-donación anónimo F4** (`validarTokenPostdonacion()`, `guardarRespuestaPostdonacion()`). Particular: tiene que funcionar sin sesión, por el anonimato.
2. **Notificaciones**. El más grande: `mensajes.json` no contempla notificaciones de donante hoy (solo hospital↔admin), hay que decidir el modelo de datos antes de codear.
...
**Verificado contra el código real (2026-09-08), no contra el estado que dice esta tabla más abajo** — hoy solo existen 3 funciones de escritura en *todo* el proyecto: `usuarios` (Donante, su propio perfil/registro), `turnos` (Donante crea, Hospital confirma/rechaza) y `formulario_consentimiento` (Donante, F1/F2). Ninguna otra tabla (`campanas`, `donaciones`, `hospitales`, `documentos`, `resultado_analisis`, `certificado_donacion`, `mensajes`, `profesionales`, `facturas`, etc.) tiene una sola función que la escriba en ningún rol — son 100% datos semilla estáticos hasta que se conecte alguna.
...
| `usuarios` | Donante: registro + `actualizarPerfilDonante()` — 🟢 (su propio registro). Super Admin: suspender/reactivar donante — 🔴 sin implementar. | Donante — 🟢. Hospital/Profesional (para mostrar nombre/DNI del donante en un turno) — 🟢/🟡. Super Admin (`cargarDashboardAdmin()`, stats) — 🟡 lectura silenciosa. |
| `donaciones` | Nadie. Profesional: `registrarDonacion()` en Atención clínica — 🟡 no persiste nada (el modal se cierra y se pierde todo). | Donante (`crearTurno()` regla 90 días, `cargarMisDonaciones()`) — 🟢. Hospital/Super Admin (dashboards) — 🟡 lectura silenciosa. |
| `formulario_consentimiento` | Donante: `guardarFormularioConsentimiento()` (F1+F2) — 🟢. Profesional: F3 (evaluación clínica, extendería el mismo registro con `donacion_id`/firmas del profesional) — 🔴 no implementado. | Donante: `cargarMisDocumentos()` (pestaña Consentimientos) — 🟢. |
...
| `resultado_analisis` / `certificado_donacion` | Nadie. Hospital — 🔴 en ambas. | Donante: `cargarMisDocumentos()` (pestañas Resultados/Certificados) — 🟢. |
