...
**Reservar turno**
- *Usuario:* elegís turno (fecha/hora) desde el detalle de la campaña y confirmás. Si donaste hace menos de 90 días, el sistema te avisa cuándo podés volver a donar. El turno queda a la espera de que el hospital lo confirme, y aparece en "Mis turnos" con recordatorio el día anterior.
- *Técnico:* valida disponibilidad en tiempo real contra `turnos`, crea registro `estado: pendiente` asociado a `usuario_id` y `campana_id` — pasa a `confirmado` cuando el hospital lo acepta (RF3, ver "Gestionar turnos del día" en el rol Administrador de hospital). Verifica período de espera activo (< 90 días desde última donación) antes de habilitar la reserva; si aplica, informa la fecha habilitada (solo posible si la extracción previa se registró en este sistema, al no estar integrado con sistemas externos).
...
**Mis turnos**
- *Usuario:* consultás turnos pasados y futuros con su estado, el estado de tus formularios pre-donación, cancelás hasta 2 horas antes, o cambiás el turno hasta 24 horas antes. Ese margen de 24hs le da al hospital tiempo de reacción operativa (reasignar el cupo liberado) antes de que llegue la fecha pactada — no es una aprobación: modificar un turno no lo vuelve a poner en revisión (corregido 2026-09-18, ver "Confirmación automática y cupo por turno" en `docs/04`). Después de ese límite, se muestran los datos de contacto del hospital.
- *Técnico:* consulta `turnos` por `usuario_id` con estados (`pendiente`, `confirmado`, `en_curso`, `completado`, `no_apto`, `ausente`, `cancelado`) y estado de formularios vinculados. Cancelación hasta 2h antes, modificación hasta 24h antes — ambas ventanas están pensadas para que el hospital tenga margen de reacción (reasignar el cupo, gestionar la confirmación) antes de que el turno sea inminente.
- *Nota sobre el estado `pendiente` (agregada 2026-08-19):* se agrega a la lista de estados de arriba porque un turno recién reservado por el donante espera confirmación del hospital antes de pasar a `confirmado` (ver RF3 y "Reservar turno" más arriba) — no estaba en la enumeración original de este párrafo, pero es parte real del ciclo de vida del turno y hay que contemplarlo al implementar `actualizarTurno()`/`cancelarTurno()` (ej.: ¿se puede modificar/cancelar un turno todavía `pendiente`, no solo uno ya `confirmado`? A definir).
- *Nota sobre el estado `ausente` (agregada 2026-08-19):* en el MVP esta marca la hace manualmente el profesional de salud, al ver que el donante no se presentó, pasado un tiempo de gracia desde la hora del turno — no hay ningún proceso automático que lo detecte hoy. **Sugerencia para una versión futura:** automatizar esta marca sin depender de que un humano entre al sistema a hacerla — por ejemplo, un turno de las 10:00 que a las 15:00 del mismo día sigue sin asistencia registrada podría marcarse `ausente` automáticamente (job programado del lado del backend).
