...
> Bandeja de notificaciones del donante — no existía ningún modelo de datos para esto antes de esta fecha (`donante/notificaciones.html` era 100% estática). Se pobló con los 6 ejemplos que ya tenía la pantalla estática como semilla real. Es también el destino de la notificación que genera `HemoRed.data.resolverSolicitudCorreccion()` al rechazar una solicitud de corrección de certificado (ver esa sección más abajo) — hoy es la única fuente dinámica real; el resto de los flujos que "deberían" generar una notificación (turno confirmado, resultado cargado, etc.) todavía no la crean, siguen siendo solo semilla.
